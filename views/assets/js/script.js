...
(function($) {
	...
	$(function() {
		...
		$("#followupcancelbtn").on("click", function(){
			$("#followupform").hide();
			$("#followupbtn").show();
		});
	...
	$(document).ready(function () {
		...
		$(".next").on("click", function () {
			...
			const nextTab = currentTab.closest("li").next("li").find(".nav-link");
		...
		$(".previous").on("click", function () {
			const currentTab = $(".nav-tabs .nav-link.active");
			const prevTab = currentTab.closest("li").prev("li").find(".nav-link");
	...
	if ($('#phone').length > 0) {
		...
		window.intlTelInput(input, {
			utilsScript: "./views/assets/plugins/intltelinput/js/utils.js",
		});
	}
